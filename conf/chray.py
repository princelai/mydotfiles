...
class ChangeV2ray:
    def __init__(self):
        ...
        self.server = {
            "kvmcn2": "ssh -p 28777 root@lunarch.top",
            "kvmqnet": "ssh -p 28777 root@solarck.top"
        }
        self.scpcmd = {
            "kvmcn2": "scp -P 28777 {} root@lunarch.top:/etc/v2ray/",
            "kvmqnet": "scp -P 28777 {} root@solarck.top:/etc/v2ray/"
        }
        key_num = int(
            input(
                'Please select fq method:\n\t1).H2CanddyTLS\n\t2).mKcp\n\t3).H2-TLS\n\t4).SS-mKcp\n\t5).SS-TLS\n:'
            ))
        ...
        self.client_files = [
            str(i) for i in p.iterdir() if i.name.endswith('client.json')
        ]
        self.server_files = [
            str(i) for i in p.iterdir() if i.name.endswith('server.json')
        ]
    ...
    def cp_files_client(self):
        ...
        child.expect('\[.+\][#$]')
        child.sendline(
            f"yes | cp -f {' '.join(self.client_files)} /etc/v2ray/")
        child.expect('\[.+\][#$]')
        child.close()
    ...
    def handle_client(self):
        ...
        child.sendline(
            f"systemctl start v2ray@{self.new_service_prefix}-client.service")
        child.expect('\[.+@.+\][#$]')
        child.sendline(
            f"systemctl enable v2ray@{self.new_service_prefix}-client.service")
        child.expect('\[.+@.+\][#$]')
        child.close()
    ...
    def handle_server(self):
        for k, v in self.server.items():
            ...
            child.sendline(
                f"systemctl start v2ray@{self.new_service_prefix}-server.service"
            )
            child.expect('.+@.+[#$]')
            child.sendline(
                f"systemctl enable v2ray@{self.new_service_prefix}-server.service"
            )
            child.expect('.+@.+[#$]')
            self.logout(child)
